...
def initialize(percentFibril = .15, 
    ...
    return excitable, refracPer, lastActivated, activateNext

def read_color_palette(path): 
    #pass your own path to the colorscale.txt file when you download it
    colorfile = open(path)
...
def iter_epochs(excitable, refracPer, lastActivated, activateNext, color_palette_path, num_epochs = 1234, stimInt = 51, aniso = 'x'):
    ...
    for epoch in range(num_epochs):#runtime = num_epochs * 2 ms
        ...
        mask = color_idx > 0
        all_coords.append(np.argwhere(mask).astype(np.float32))
        all_colors.append((color_matrix[color_idx[mask]]*255).astype(np.uint8))
    ...
    cloud = pv.PolyData(all_coords[0])
    cloud.point_data['colors'] = all_colors[0]
    actor = plotter.add_mesh(cloud, scalars='colors', rgb=True, point_size=10, render_points_as_spheres=True)

    def update_epoch(value):
        epoch = int(value)
        new_cloud = pv.PolyData(all_coords[epoch])
        new_cloud.point_data['colors'] = all_colors[epoch]
        #new_cloud['rgb'] = all_colors[epoch]
        actor.mapper.dataset.copy_from(new_cloud)
        plotter.render()

    plotter.add_slider_widget(update_epoch, rng=[0, num_epochs - 1],
                          value=50, title='Epoch', color='white')
    plotter.show()


def main():
    excitable, refracPer, lastActivated, activateNext = initialize(percentFibril=.3, defaultRefractoryPeriod=40, patchRefractoryPeriod=55)
    iter_epochs(excitable, refracPer, lastActivated, activateNext, color_palette_path='/Users/ashnagibbons/Documents/CMP487/colorscale.txt')
